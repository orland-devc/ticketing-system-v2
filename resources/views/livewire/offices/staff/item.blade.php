...
<div x-data="{ editModal: false, editBgModal: false, deleteModal: false, deleteBgModal: false }" class="max-w-screen">
            ...
            <div class="flex-1 min-w-0 truncate">
                ...
                @if ( $user->office?->name )
                    <p class="text-sm text-gray-600 dark:text-gray-400 flex items-center gap-1.5 mt-0.5">
                        <i class="fas fa-briefcase text-xs text-blue-500"></i>
                        <span class="truncate">{{ $user->office?->name }}</span>
                    </p>    
                @else
                    <p class="text-sm text-red-600 dark:text-red-400 flex items-center gap-1.5 mt-0.5">
                        <i class="fas fa-briefcase text-xs"></i>
                        <span class="truncate">Office not assigned</span>
                    </p>
                @endif
            </div>
    ...
    <livewire:offices.staff.edit :user="$user" />
</div>